...
AMREX_GPU_DEVICE ScalarVectorTensor
FourDerivScalarTensor<coupling_and_potential_t, deriv_t>::compute_M_Ni_and_Mij(
    ...
    const Tensor::Rank2 &h_UU) const
{
    ...
    FOR(i, j)
    {
        ...
        {
            out.tensor(i, j) +=
                -vars.A(i, k) * vars.A(j, l) * h_UU(k, l) / vars.chi();
        }
    }
...
template <class coupling_and_potential_t, class deriv_t>
AMREX_GPU_DEVICE ScalarVectorTensor
FourDerivScalarTensor<coupling_and_potential_t, deriv_t>::compute_Omega_munu(
    const int ix, const int iy, const int iz,
    const amrex::Array4<const amrex::Real> &state, const deriv_t &a_deriv,
    const Tensor::Rank2 &h_UU) const
{
    ...
    m_coupling_and_potential.compute_coupling_and_potential(
        dfdphi, d2fdphi2, V_of_phi, dVdphi, g2, dg2dphi, vars);
    ...
    FOR(i, j)
    {
        ...
        covd2phi(i, j) += 0.5 *
                          (d1_phi(i) * d1_chi(j) + d1_phi(j) * d1_chi(i) -
                           vars.h(i, j) * dphi_dot_dchi) /
                          vars.chi();
    }
    ...
    {
        out.vector(i) =
            -4.0 * d2fdphi2 * vars.Pi() * d1_phi(i) +
            4.0 * dfdphi * (-d1_Pi(i) - vars.K() * d1_phi(i) / GR_SPACEDIM);
        FOR(j, k)
        {
            out.vector(i) +=
                -4.0 * dfdphi * h_UU(j, k) * d1_phi(k) * vars.A(i, j);
        }
    }
    return out;
}
...
template <class coupling_and_potential_t, class deriv_t>
AMREX_GPU_DEVICE RhoAndJ
FourDerivScalarTensor<coupling_and_potential_t, deriv_t>::compute_rho_and_j(
    const int ix, const int iy, const int iz,
    const amrex::Array4<const amrex::Real> &state, const deriv_t &a_deriv,
    const Tensor::Rank2 &h_UU) const
{
    ...
    ScalarVectorTensor SVT =
        compute_M_Ni_and_Mij(ix, iy, iz, state, a_deriv, h_UU);
    ...
    FOR(i, j, k)
    {
        ...
        covd_Aphys_times_chi(j, k, i) +=
            0.5 * (vars.A(i, k) * d1_chi(j) + vars.A(i, j) * d1_chi(k)) /
            vars.chi();
        FOR(l, m)
        {
            covd_Aphys_times_chi(j, k, i) +=
...
template <class coupling_and_potential_t, class deriv_t>
AMREX_GPU_DEVICE S_TFAndTrS
FourDerivScalarTensor<coupling_and_potential_t, deriv_t>::compute_S_TF_and_trS(
    const int ix, const int iy, const int iz,
    const amrex::Array4<const amrex::Real> &state, const deriv_t &a_deriv,
    const Tensor::Rank2 &h_UU) const
{
    ...
    auto advec_A =
        a_deriv.advec_sym_tensor(ix, iy, iz, state, shift_vector, c_A11);
    ...
    FOR(i, j)
    {
        out.S_TF(i, j) =
            ...
            g2 * Vt *
                (-d1_phi(i) * d1_phi(j) + vars.h(i, j) / vars.chi() * Vt / 4.0);
    }
    ...
    FOR(i, j)
    {
        ...
        FOR(k)
        covd2lapse_times_chi(i, j) +=
            -vars.chi() * chris.ULL(k, i, j) * d1_lapse(k);
        covd2lapse_times_chi(i, j) +=
            0.5 * (d1_lapse(i) * d1_chi(j) + d1_lapse(j) * d1_chi(i) -
                   vars.h(i, j) * dlapse_dot_dchi);
    }
    ...
    FOR(i, j)
    {
        ...
        covd2phi_times_chi(i, j) +=
            -vars.chi() * chris.ULL(k, i, j) * d1_phi(k);
        covd2phi_times_chi(i, j) +=
            0.5 * (d1_phi(i) * d1_chi(j) + d1_phi(j) * d1_chi(i) -
                   vars.h(i, j) * dphi_dot_dchi);
    }
    ...
    {
        Fij(i, j) =
            (-advec_A(i, j) + covd2lapse_times_chi(i, j)) / vars.lapse() -
            2.0 / 3.0 * vars.A(i, j) * (vars.K() - divshift / vars.lapse());
        FOR(k)
        {
            Fij(i, j) += -(vars.A(k, i) * d1_shift(k, j) +
                           vars.A(k, j) * d1_shift(k, i)) /
                         vars.lapse();
            FOR(l) { Fij(i, j) += h_UU(k, l) * vars.A(i, k) * vars.A(l, j); }
        }
    }
    ...
    amrex::Real F = (-advec_K + tr_covd2lapse) / vars.lapse() - Aij_squared -
                    vars.K() * vars.K() / 3.0;
    ...
    FOR(i, j)
    {
        ...
        {
            RGB +=
                -8.0 * vars.chi() * h_UU(i, l) * h_UU(j, m) * h_UU(k, n) *
    ...
    // for Pi in Sij and S
    amrex::Real quadratic_terms =
        -dVdphi - 3.0 / 4.0 * dg2dphi * Vt * Vt -
        2.0 * g2 * vars.Pi() * vars.K() * vars.Pi() * vars.Pi() +
        2.0 / 3.0 * g2 * vars.Pi() * vars.K() * (Vt + vars.Pi() * vars.Pi());
    FOR(i, j)
    {
        quadratic_terms +=
            2.0 * g2 * vars.Pi() * vars.Pi() * h_UU(i, j) *
                (0.5 * d1_chi(j) * d1_phi(i) - vars.chi() * d2_phi(i, j)) +
            4.0 * g2 * h_UU(i, j) * vars.chi() * d1_phi(j) * vars.Pi() *
                d1_Pi(i);
        FOR(k)
        {
            ...
            {
                quadratic_terms +=
                    2.0 * g2 * h_UU(i, k) * h_UU(j, l) * vars.chi() *
                    d1_phi(k) * d1_phi(l) *
    ...
    quadratic_terms *=
        dfdphi / (1.0 + g2 * (-Vt + 2.0 * vars.Pi() * vars.Pi()));
                             ...
                             const Tensor::Rank2 &h_UU) const
{
    ...
    FOR(i) { out.j(i) = coupling * rho_and_j.j(i); }
    FOR(i, j) { out.S_TF(i, j) = coupling * S_TF_and_trS.S_TF(i, j); }
    ...
    const amrex::Array4<const amrex::Real> &state, const deriv_t &a_deriv) const
{
    ...
    {
        Fij_times_lapse(i, j) =
            -advec_A(i, j) + covd2lapse_times_chi(i, j) -
            2.0 / 3.0 * vars.A(i, j) * (vars.lapse() * vars.K() - divshift);
        FOR(k)
        {
            Fij_times_lapse(i, j) += -(vars.A(k, i) * d1_shift(k, j) +
                                       vars.A(k, j) * d1_shift(k, i));
            FOR(l)
            {
                Fij_times_lapse(i, j) +=
    ...
    amrex::Real F_times_lapse =
        -advec_K + tr_covd2lapse -
    ...
    {
        RGB_times_lapse +=
            8.0 * Mij_TF_UU_over_chi(i, j) * Fij_times_lapse(i, j) +
        ...
        {
            RGB_times_lapse +=
                -8.0 * vars.chi() * vars.lapse() * h_UU(i, l) * h_UU(j, m) *
                h_UU(k, n) * covd_Aphys_times_chi(m, n, l) *
                (covd_Aphys_times_chi(j, k, i) - covd_Aphys_times_chi(i, j, k));
        }
    }
...
FourDerivScalarTensor<coupling_and_potential_t, deriv_t>::compute_lhs(
    ...
    const deriv_t &a_deriv, amrex::Real *LHS) const
{
    ...
    FOR2_SYM(i1, j1)
    {
        ...
        {
            LHS_mat[row][col] =
                -(2 - TensorAlgebra::delta(i1, j1)) * 4.0 * coupling *
                (vars.h(i2, j2) / 3.0 * Omega_ij_TF_UU_over_chi(i1, j1) +
                 16.0 * vars.chi() * dfdphi2 * Mij_TF(i2, j2) *
                     Mij_TF_UU_over_chi(i1, j1));
            LHS_mat[row][col] +=
                (1.0 - 2.0 * coupling * Omega / 3.0) *
                (TensorAlgebra::delta(i1, i2) * TensorAlgebra::delta(j1, j2) +
                 (1 - TensorAlgebra::delta(i1, j1)) *
                     TensorAlgebra::delta(i1, j2) *
                     TensorAlgebra::delta(i2, j1));
            FOR(k)
            {
                LHS_mat[row][col] +=
                    2.0 * coupling * vars.chi() *
                    ((TensorAlgebra::delta(i1, i2) * h_UU(j1, k) *
                          Omega_ij_TF(j2, k) +
                      TensorAlgebra::delta(j1, j2) * h_UU(i1, k) *
                          Omega_ij_TF(i2, k)) +
                     (1 - TensorAlgebra::delta(i1, j1)) *
                         (TensorAlgebra::delta(i1, j2) * h_UU(j1, k) *
                              Omega_ij_TF(i2, k) +
                          TensorAlgebra::delta(j1, i2) * h_UU(i1, k) *
                              Omega_ij_TF(j2, k)));
            }
            ++col;
        }
    ...
    FOR2_SYM(i, j)
    {
        LHS_mat[matrix_dim - 2][idx] =
            2.0 * coupling * vars.chi() / 3.0 *
            (-Omega_ij_TF(i, j) + 16.0 * dfdphi2 * M * Mij_TF(i, j));
        LHS_mat[idx][matrix_dim - 2] =
            (2 - TensorAlgebra::delta(i, j)) * coupling *
            (Omega_ij_TF_UU_over_chi(i, j) -
             16.0 * dfdphi2 * M * Mij_TF_UU_over_chi(i, j));
        ++idx;
    }
    LHS_mat[matrix_dim - 2][matrix_dim - 2] =
        1.0 + 2.0 * coupling / 3.0 * (-Omega + 4.0 * dfdphi2 * M * M);

    idx = 0;
    FOR2_SYM(i, j)
    {
        LHS_mat[matrix_dim - 1][idx] = 0.0;
        LHS_mat[idx][matrix_dim - 1] = -(2 - TensorAlgebra::delta(i, j)) * 8.0 *
                                       dfdphi * Mij_TF_UU_over_chi(i, j);
        ++idx;
    }
    LHS_mat[matrix_dim - 1][matrix_dim - 2] = 0.0;
    LHS_mat[matrix_dim - 2][matrix_dim - 1] = 4.0 * dfdphi * M / 3.0;
    LHS_mat[matrix_dim - 1][matrix_dim - 1] =
        1.0 + g2 * (2.0 * vars.Pi() * vars.Pi() - Vt);
