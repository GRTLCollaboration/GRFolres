...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void LU_decompose(const int &N,
                                                      amrex::Real *A)
{
    for (int i = 0; i < N; i++)
    {
...
forward_substitution(const int &N, const amrex::Real *L, amrex::Real *b)
{
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < i; j++)
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void
back_substitution(const int &N, const amrex::Real *U, amrex::Real *b)
{
    for (int i = N - 1; i >= 0; i--)
    {
        for (int j = i + 1; j < N; j++)
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void
solve_linear_system(const int &N, amrex::Real *A, amrex::Real *b)
{
    LU_decompose(N, A);
