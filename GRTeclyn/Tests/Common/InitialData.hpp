                         ...
                         const amrex::RealVect &a_coords)
{
    ...
    // B is set to zero: the integrated moving-puncture gauge stores the initial
    // Gamma-driver RHS in B and subtracts it from the shift RHS, which is only
    // meaningful for genuine initial data. Zeroing it here makes the shift RHS
    // directly comparable with the (non-integrated) GRChombo gauge.
    a_array(a_iv, c_B1) = 0.0;
    a_array(a_iv, c_B2) = 0.0;
    a_array(a_iv, c_B3) = 0.0;
    // NOLINTEND(cppcoreguidelines-avoid-c-arrays,modernize-avoid-c-arrays)
}

// Polynomial scalar-field initial data for the 4dST tests.
AMREX_GPU_DEVICE AMREX_FORCE_INLINE void
fdst_scalar_initial_data(const amrex::IntVect &a_iv,
                         const amrex::Array4<amrex::Real> &a_array,
                         const amrex::RealVect &a_coords)
{
    amrex::Real x = a_coords[0];
