    ...
    const Vars<data_t> &advec, const Coordinates<data_t> &coords) const
{
    ...
    auto ricci0 = CCZ4Geometry::compute_ricci_Z(matter_vars, d1, d2, h_UU,
                                                chris, {0., 0., 0.});
    ...
    FOR(i, j, k)
    {
        Ni[i] += h_UU[j][k] * (covdtilde_A[k][j][i] -
                               GR_SPACEDIM * matter_vars.A[i][j] * d1.chi[k] /
                                   (2. * chi_regularised));
    }
    ...
    FOR(i)
    {
        ...
        FOR(j)
        {
            ...
            FOR(k)
            {
                b_term_gamma += b_of_x / (1. + b_of_x) * 2. *
                                       matter_vars.lapse * A_UU[i][j] *
                                       matter_vars.h[k][j] * Z_over_chi[k];
            }
        }
